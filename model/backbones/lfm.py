...
class Block(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, h=14, w=8):
        ...
        self.filter = FilterLayer(dim, h=h, w=w)
    ...
    def forward(self, x):
        ...
        # print('*'*10, x.shape) # torch.Size([10, 196, 512])
        return x
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=196, patch_size=16, in_chans=3, embed_dim=768):
        super().__init__()
    ...
    def forward(self, x):
        # shape torch.Size([1, 3, 224, 224]) 1 times
        B, C, H, W = x.shape
...
class LFM(nn.Module):
    ...
    def __init__(self, pretrained=None, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
    ...
    def load_param(self, model_path):
        ...
        for k, v in param_dict.items():
            ...
            if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
                ...
                print('x'*30, v.shape, self.pos_embed.shape)
...
def lfm_reid_base(pretrained=None, imge_size=224, patch_size=16, embed_dim=512, mlp_ration=4, depth=19, drop_path_rate=0., drop_rate=0.):
    model = LFM(
        pretrained=pretrained,
        img_size=imge_size,
        patch_size=patch_size, embed_dim=embed_dim, depth=depth, mlp_ratio=mlp_ration, drop_path_rate=drop_path_rate,
